...
import type { ApiErrorBody, ApiErrorItem } from "../response/api-response.factory";
...
function mapValidationEntry(entry: unknown): ApiErrorItem {
  ...
  if (typeof e.property === "string" && e.constraints && typeof e.constraints === "object") {
...
export class HttpExceptionFilter implements ExceptionFilter {
  ...
  private normalizeHttpPayload(payload: unknown, fallbackMessage: string): ApiErrorBody {
    ...
    if (Array.isArray(messageRaw)) {
      const errors = messageRaw.map((entry) =>
        typeof entry === "string" ? { message: entry } : mapValidationEntry(entry),
      );
      return {
        success: false,
