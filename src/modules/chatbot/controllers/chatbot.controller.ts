...
export class ChatbotController {
  ...
  async handleWebhook(@Body() payload: any) {
    try {
      ...
      if (!event) {
        console.log("Sin evento en payload, intentando procesar como mensaje directo");
  ...
  async handleMessagesUpsert(@Body() payload: any) {
    try {
      ...
      console.log("Extracted messageData:", JSON.stringify(messageData, null, 2));
  ...
  async handleMessagesUpdate(@Body() payload: any) {
    try {
      console.log("Messages update recibido:", JSON.stringify(payload, null, 2));
      return { success: true };
    } catch (error) {
  ...
  async handleConnectionUpdate(@Body() payload: any) {
    console.log("Connection update recibido:", JSON.stringify(payload, null, 2));
    return { success: true };
  }
  ...
  private async processMessageData(messageData: any): Promise<void> {
    ...
    const remoteJid = messageData.key?.remoteJidAlt || messageData.key?.remoteJid || messageData.remoteJid;
